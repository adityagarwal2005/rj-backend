...
class PaymentSerializer(serializers.ModelSerializer):
    class Meta:
        model = Payment
        fields = ["id", "order", "gateway", "status", "amount", "currency", "utr_reference", "created_at"]
...
class SubmitUtrSerializer(serializers.Serializer):
    """Customer records their UPI transaction reference for a pending manual payment."""

    order_id = serializers.PrimaryKeyRelatedField(source="order", queryset=Order.objects.all())
    utr_reference = serializers.CharField(min_length=4, max_length=35, trim_whitespace=True)
